...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await db`
      INSERT INTO products (id, client_id, name, sku, description, category, price, sale_price, stock, low_stock_threshold, status, image)
      VALUES (${id}, ${session.clientId}, ${name}, ${sku ?? null}, ${description ?? null}, ${category ?? null}, ${price ?? 0}, ${sale_price ?? null}, ${stock ?? 0}, ${low_stock_threshold ?? 10}, ${status ?? 'draft'}, ${image ?? null})`;
